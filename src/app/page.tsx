...
export default function Home() {
  ...
  const navClass = useMemo(
    () =>
      `fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
        scrolled
          ? 'border-b border-white/10 shadow-[0_12px_36px_rgba(0,0,0,0.32)] backdrop-blur-xl'
          : 'bg-transparent'
      }`,
    [scrolled]
  );
  ...
  return (
    <main className='relative min-h-screen overflow-x-hidden bg-[var(--editor-shell-bg)] text-white'>
      ...
      <header className={navClass} style={navStyle}>
        <div className='mx-auto flex h-16 max-w-6xl items-center justify-between px-5'>
          <div className='flex items-center gap-2'>
            <span className='inline-flex h-8 w-8 items-center justify-center rounded-lg border border-white/12 bg-white/8 text-sm font-semibold'>
              E
            </span>
            <span className='text-sm font-semibold tracking-[0.12em] text-white/90'>EASYRESUME</span>
          </div>
          <Link
            href='/edit'
            className='inline-flex h-9 items-center justify-center rounded-full border border-white/16 bg-white/10 px-4 text-sm font-medium text-white/92 transition-transform duration-200 hover:-translate-y-px hover:bg-white/16 text-white!'
          >
            立即开始
          </Link>
        </div>
          ...
          <div className='space-y-6'>
            <p
              ...
              style={{
                ...
                color: 'color-mix(in srgb, var(--color-primary-gradient-start) 68%, white)',
              }}
            >
              SMART RESUME EDITOR
            </p>
            <h1 className='max-w-[14ch] text-4xl font-semibold leading-[1.08] text-white/95 md:text-6xl'>
              快速编辑简历，稳定导出，精准投递
            </h1>
            <p className='max-w-[62ch] text-base leading-7 text-white/64'>
              面向求职者打造的高效简历编辑器。模块化编辑、AI 智能建议与多格式导出协同工作，帮你在最短时间内完成更专业的简历版本。
            </p>
            <div className='flex flex-wrap items-center gap-4'>
              <Link
                href='/edit'
                className='group relative inline-flex h-12 min-w-[158px] items-center justify-center overflow-hidden rounded-xl border border-white/20 px-6 text-sm font-semibold text-white shadow-[0_16px_40px_rgba(0,0,0,0.35)] transition-transform duration-200 hover:-translate-y-[2px]'
      ...
      <section id='features' className='mx-auto w-full max-w-6xl px-5 py-16 md:py-20'>
        ...
        >
          <h2 className='text-2xl font-semibold text-white/94 md:text-3xl'>核心功能特性</h2>
          <p className='mt-2 max-w-[64ch] text-sm leading-7 text-white/58'>
        ...
        <div className='grid gap-4 md:grid-cols-3'>
          {FEATURES.map((item, idx) => (
            <article
              ...
              className={`group rounded-2xl border border-white/12 bg-[rgba(17,20,40,0.58)] p-5 shadow-[0_16px_36px_rgba(0,0,0,0.25)] backdrop-blur-lg transition-all duration-500 hover:-translate-y-[4px] ${revealClass(
                `feature-${idx}`
              )}`}
              style={{
                borderColor: 'color-mix(in srgb, var(--color-primary-gradient-start) 18%, rgba(255,255,255,0.12))',
              }}
            >
              <div className='mb-4 h-[2px] w-16' style={{ background: 'var(--gradient-primary)' }} />
              <h3 className='text-base font-semibold text-white/92'>{item.title}</h3>
