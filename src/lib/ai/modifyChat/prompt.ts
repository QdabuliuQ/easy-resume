...
只要用户是在指示如何改简历（即使未出现「简历」二字），一律判 modify_resume。
...
输出格式：
{{"scope":"partial"|"global"|"ambiguous","targets":[{{"moduleId":"模块id","itemIndex":0}}],"scene":"work"|"project"|"skill"|null,"action":"polish"|"edit"|"add"|"remove"|"auto","clarifyMessage":"仅 ambiguous 时填写"}}
...
- 用户提到的模块名若已在清单 label 中出现，必须指向对应 moduleId，禁止建议新增模块
- 用户要求删除整个模块时 action 为 remove，targets 指向该 moduleId，勿填 itemIndex
- 禁止新增 modules；允许删除 modules；ambiguous 时列出清单中各 label 供用户选择
