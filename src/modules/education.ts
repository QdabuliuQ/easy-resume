...
function createEducationItem(props: EducationItemProps, header: fabric.Group, globalStyle: Partial<GlobalStyle>) {
    ...
    for (const tag of tags) {
        ...
        if (tGroup.length > 0) {
            tagGroup.set({
                left: (tGroup[tGroup.length - 1].left ?? 0) + (tGroup[tGroup.length - 1].width ?? 0) + columnMargin,
            })
        } else {
            tagGroup.set({
    ...
    const tagsGroups = new fabric.Group(tGroup, {
        top: (header.top ?? 0) + (header.height ?? 0) + rowMargin - ((tGroup[0].height ?? 0) - (schoolText.height ?? 0)) / 2,
        left: (typeText.left ?? 0) + (typeText.width ?? 0) + columnMargin,
    });
...
export default function createEducationModule(props: EducationProps) {
    ...
    const educationModule = new fabric.Group([
        header,
        ...groups,
    ], {
